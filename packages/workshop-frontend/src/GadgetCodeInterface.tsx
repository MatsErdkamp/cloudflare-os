...
import { useToast, Button } from '@matser/ui'
...
export default function GadgetCodeInterface({ overseer, filesRoot, height = '100%', onCodeChange, selectedChatId = null, proposedChanges, draftProposedChanges, streamingProposedChanges, streamingActiveFile, isAgentActive, isVisible = true, onHasCodeChange }: GadgetCodeInterfaceProps) {
    ...
    const subscribe = async () => {
      try {
        ...
        if (!isReadyRef.current) {
          toasts.add({ title: 'Failed to load code files', type: 'error' })
          setLoading(false)
        }
  ...
  const handleFileCreate = (filename: string) => {
    ...
    if (filesMap.has(filename)) {
      toasts.add({ title: `File already exists: ${filename}`, type: 'error' })
      return
    }
    ...
    setActiveFile(filename)
    toasts.add({ title: `Created file: ${filename}`, type: 'success' })
  }
  ...
  const handleFileDelete = (filename: string) => {
    ...
    if (!filesMap.has(filename)) {
      toasts.add({ title: 'File not found', type: 'error' })
      return
    }
    ...
    toasts.add({ title: `Deleted file: ${filename}`, type: 'success' })
  }
  ...
  const handleFileRename = (oldName: string, newName: string) => {
    ...
    if (filesMap.has(newName)) {
      toasts.add({ title: `File already exists: ${newName}`, type: 'error' })
      return
    }
    ...
    toasts.add({ title: `Renamed file: ${oldName} \u2192 ${newName}`, type: 'success' })
  }
  ...
  const handleFileDownload = useCallback((filename: string) => {
    ...
    if (!ytext) {
      toasts.add({ title: `Could not download ${filename}`, type: 'error' })
      return
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height, width: '100%' }}>
      {hasUnsavedChanges && (
        <div className="bg-muted border-b border-border px-4 py-2 flex items-center gap-2 text-sm text-status-warning">
