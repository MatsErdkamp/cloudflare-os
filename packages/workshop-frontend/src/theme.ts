...
//
// The base light/dark palettes come from @matser/ui/styles.css. Theme mode is applied on <html>
// using both the UI package's `.dark` selector and the app's `data-mode` attribute so semantic
...
// follow the light/dark palettes selected by the UI package. The seed is always a
...
function accentVars(seed: string): Record<string, string> {
  return {
    '--primary': `light-dark(${seed}, oklch(from ${seed} 0.45 c h))`,
    '--color-primary': `light-dark(${seed}, oklch(from ${seed} 0.45 c h))`,
